...
part 'async_custom_paginated_data_table.dart';

/// Allows to externally control [CustomPaginatedDataTable] state
/// and trigger actions such as changing page number or size. Instantiate an object,
/// keep it somewhere (e.g. parent widgets state or static field/top level variable),
/// pass it to [CustomPaginatedDataTable.controller] via constructor and you're ready to go.
...
/// method of [CustomPaginatedDataTable].
/// The controller extends [ChangeNotifier] in order to let consumers know
/// if there're changes to [CustomPaginatedDataTable] state. E.g. you can hide
/// standard paginator of [CustomPaginatedDataTable] and implement your own
/// paginator as a StatefulWidget, subscribe to controller in order to update
/// the paginator.
class PaginatorController extends ChangeNotifier {
  CustomPaginatedDataTableState? _state;

  // Whenever setState is called within PaginatedDataTable2State and there's
  // an attached controller, than this method is called by PaginatedDataTable2State
  void _notifyListeners() {
    notifyListeners();
  }

  /// The controller is attached to [CustomPaginatedDataTable] state upon
  /// the first build. Until data from internal stare is not available
  bool get isAttached => _state != null;

  void _attach(CustomPaginatedDataTableState state) {
    _state = state;
  }
  ...
  /// Returns number of rows displayed in the [CustomPaginatedDataTable]. Throws if no
  ...
  /// Returns number of rows displayed in single page of the [CustomPaginatedDataTable].
  ...
  /// Returns the index of the first (topmost) row displayed currently displayed in [CustomPaginatedDataTable].
  /// Throws if no table is attached to the controller
  ...
  /// Change page size and set the number of rows in a single page
...
/// Has the header row and paginator always fixed to top and bottom (correspondingly).
...
/// relative column sizes (setting them to S, M and L). [CustomDataColumn] provides
/// row-level tap event handlers.
/// See also:
///
///  • [CustomDataTable], which is not paginated.
class CustomPaginatedDataTable extends StatefulWidget {
  /// Check out [PaginatedDataTable] for the API description.
  ...
  /// When there're few rows it determines if the core
  /// of the table must grow and fill the container (FlexFit.tight - useful if
  ...
  /// your own paginator and control the widget via [CustomPaginatedDataTable.controller]
  final bool hidePaginator;

  /// Used to control widget's state externally and trigger actions. See
  /// [PaginatorController]
...
class CustomPaginatedDataTableState extends State<CustomPaginatedDataTable> {
  ...
  void setState(VoidCallback fn) {
    ...
    if (widget.controller != null) {
      Future.delayed(Duration(), () => widget.controller?._notifyListeners());
    }
  ...
  void f(int r) {
    ...
    if (widget.onRowsPerPageChanged != null) {
      widget.onRowsPerPageChanged?.call(r);
    }
  }
      ...
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        if (widget.autoRowsToHeight) {
          ...
          if (_prevRowsPerPageForAutoRows != _effectiveRowsPerPage) {
            ...
            // how many rows were auto-calculated
