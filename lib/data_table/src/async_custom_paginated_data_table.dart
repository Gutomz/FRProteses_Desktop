part of 'custom_paginated_data_table.dart';
...
/// Please override [AsyncDataTableSource.getRows] and [DataTableSource.selectedRowCount]
...
abstract class AsyncDataTableSource extends DataTableSource {
  ...
  /// are de-selected - this can be useful if you deal with scenarios when you need
  /// to have many more selected items than deselected (e.g. selecting all rows across
  /// hundreds of pages and than deselecting certain indexes).
  ...
  /// Lists rows (their keys) which are treated as either selected or deselected (see [selectionState])
  ...
  // set row's selected property in accordance with included/excluded from selection items
  ...
  void setRowSelection(LocalKey rowKey, bool selected) {
    final i = _rows.indexWhere((r) => r.key == rowKey);
  ...
  /// and instantiate update workflow (and thus rebuilding of [AsyncPaginatedDataTable2]
  ...
  Future _fetchData(int startIndex, int count,
    ...
    Future<void> fetch() async {
      try {
        ...
        final data = await _fetchOpp!.value;
  ...
  DataRow? getRow(int index) {
    ...
    if (outputIndex - _firstRowAbsoluteIndex < 0 ||
        outputIndex >= _rows.length + _firstRowAbsoluteIndex) return null;
    outputIndex -= _firstRowAbsoluteIndex;
    _fixSelectedState(outputIndex);

    return _rows[outputIndex];
  }
...
/// returning data rows wrapped in [Future]. Provides a straightforward way
/// of integrating data table with remote back-end and is loaded with
/// convenience features such as error handling, reloading etc.
class AsyncPaginatedDataTable2 extends CustomPaginatedDataTable {
  ...
  /// Widget that is going to be displayed while loading is in progress
  /// If no widget is specified the following default widget will be displayed:
  /// ```
  /// Center(
  ///   child: SizedBox(
  ...
  /// E.g. data source failed to load data
  final Widget Function(Object? error)? errorBuilder;

  /// Should a data source return less rows than required to fill the current
  /// page of the table (e.g. when regressing with a new filter value),
  ...
  /// 3. Make another request fetching the very last page (e.g. rows 41 - 45 of 45)
...
class AsyncPaginatedDataTable2State extends CustomPaginatedDataTableState {
  ...
  void pageTo(int rowIndex, [bool align = true]) {
    if (_operationInProgress == _TableOperationInProgress.none) {
      ...
      final source = widget.source as AsyncDataTableSource;
  ...
  Widget build(BuildContext context) {
    final source = widget.source as AsyncDataTableSource;
    final w = widget as AsyncPaginatedDataTable2;

    Widget loading() {
      final x = super.build(context);
