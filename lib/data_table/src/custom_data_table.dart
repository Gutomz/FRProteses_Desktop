...
/// - see [CustomDataTable.smRatio], [CustomDataTable.lmRatio] and same properties on [PaginatedDataTable2].
...
class CustomDataTable extends DataTable {
  ...
  Widget _buildHeadingCell({
    ...
    final ThemeData themeData = Theme.of(context);
    Widget outputLabel = label;
    outputLabel = Row(
      textDirection: numeric ? TextDirection.rtl : null,
      children: <Widget>[
        Flexible(child: outputLabel),
  ...
  Widget _buildDataCell({
    ...
    if (showEditIcon) {
      ...
      outputLabel = Expanded(child: outputLabel);
      outputLabel = Row(
        textDirection: numeric ? TextDirection.rtl : null,
        children: <Widget>[outputLabel, icon],
      );
    }
    ...
    outputLabel = Container(
      ...
      child: DefaultTextStyle(
        ...
        child: DropdownButtonHideUnderline(child: outputLabel),
      ),
    );
    if (onTap != null ||
        onDoubleTap != null ||
  ...
  Widget build(BuildContext context) {
    ...
    final builder = LayoutBuilder(builder: (context, constraints) {
      int rowIndex;
      ...
      final totalColWidth = availableWidth -
          effectiveHorizontalMargin -
          (displayCheckboxColumn
              ? effectiveHorizontalMargin / 2
              : effectiveHorizontalMargin);

      final columnWidth = totalColWidth / columns.length;
      var totalWidth = 0.0;
      ...
      final ratio = totalColWidth / totalWidth;
      ...
      for (int dataColumnIndex = 0;
        ...
        for (final DataRow row in rows) {
          ...
          tableRows[rowIndex].children![displayColumnIndex] = _buildDataCell(
            ...
            onRowLongPress: row is CustomDataRow ? row.onLongPress : null,
            onRowSecondaryTap: row is CustomDataRow ? row.onSecondaryTap : null,
    ...
    sw.stop();
    if (!kReleaseMode) {
      // print('CustomDataTable built: ${sw.elapsedMilliseconds}ms');
    }
    return builder;
  }
}
...
class _SortArrowState extends State<_SortArrow> with TickerProviderStateMixin {
  ...
  _SortArrowState(bool? up) : _up = up;
